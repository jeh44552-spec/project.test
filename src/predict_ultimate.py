...
# --- 궁극의 모델 로드 ---
try:
    model = tf.keras.models.load_model('../models/ultimate_model.h5')
    class_names = np.load('../models/ultimate_class_names.npy')
    class_names = [c if c != 'times' else 'x' for c in class_names]
    print("궁극의 최종 모델 로드 성공!")
except OSError as e:
    print(f"모델 로딩 실패: {e}. 'train_ultimate.py'를 먼저 실행해 주세요.")
    exit()
...
while True:
    ...
    cv2.imshow('The Ultimate Model', display_frame)

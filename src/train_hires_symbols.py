...
DATASET_PATH = '../hires_symbols' # 오직 기호 데이터만 사용!
IMG_HEIGHT = 45
IMG_WIDTH = 45
...
print("학습할 기호:", class_names)

if not os.path.exists('../models'): os.makedirs('../models')
np.save('../models/hires_symbol_class_names.npy', class_names)

AUTOTUNE = tf.data.AUTOTUNE
train_ds = train_ds.cache().shuffle(1000).prefetch(buffer_size=AUTOTUNE)
val_ds = val_ds.cache().prefetch(buffer_size=AUTOTUNE)

# 고해상도 기호 인식을 위한 CNN 모델
...
model.save('../models/hires_symbol_model.h5')
print("고해상도 기호 전문가 모델이 'models/hires_symbol_model.h5'에 저장되었습니다.")